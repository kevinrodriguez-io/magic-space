/**
 * Code taken from @coral-xyz/anchor project,
 * modified to work without the anchor crate.
 *
 * Source:
...
/// Implements a `Space` trait for a struct or enum.
///
...
///    #[account(init, payer = payer, space = 8 + ExampleAccount::MAGIC_SPACE)]
///    pub data: Account<'info, ExampleAccount>,
/// }
/// ```
#[proc_macro_derive(MagicSpace, attributes(max_len))]
pub fn derive_init_space(item: TokenStream) -> TokenStream {
    ...
    let process_struct_fields = |fields: Punctuated<Field, Comma>| {
        ...
        quote! {
            ...
            impl #impl_generics magic_space::Space for #name #ty_generics #where_clause {
                const MAGIC_SPACE: usize = 0 #(+ #recurse)*;
            }
        }
    };
    ...
    let expanded: TokenStream2 = match input.data {
        ...
        syn::Data::Enum(enm) => {
            ...
            quote! {
                #[automatically_derived]
                impl magic_space::Space for #name {
                    const MAGIC_SPACE: usize = 1 + #max;
                }
            }
        }
        _ => unimplemented!(),
...
fn gen_max<T: Iterator<Item = TokenStream2>>(mut iter: T) -> TokenStream2 {
    if let Some(item) = iter.next() {
        ...
        quote!(magic_space::__private::max(#item, #next_item))
    } else {
        quote!(0)
    }
}
    ...
    match ty {
        ...
        Type::Path(ty_path) => {
            ...
            match type_name.as_str() {
                ...
                _ => {
                    let ty = &ty_path.path;
                    quote!(<#ty as magic_space::Space>::MAGIC_SPACE)
                }
            }
        }
